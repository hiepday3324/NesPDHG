...
class rHPDHG(raPDHG):
    ...
    def take_step(
        ...
        # w = 0
        # gamma = 1
        weight = (solver_state.solutions_count + 1) / (solver_state.solutions_count + 2)
        next_primal_solution = (
            weight * (solver_state.current_primal_solution +  delta_primal)
            + (1 - weight) * solver_state.initial_primal_solution
        )
        next_primal_product = (
            weight * (solver_state.current_primal_product +  delta_primal_product)
            + (1 - weight) * solver_state.initial_primal_product
        )
        next_dual_solution = (
            weight * (solver_state.current_dual_solution +  delta_dual)
    ...
    def perform_restart(
        ...
        weight = 1 / (solver_state.solutions_count + 1)
