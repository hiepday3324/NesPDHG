...
class nes3_pdhg(raPDHG):
    ...
    def take_step(
        ...
        # weight = (new_weights_sum) / (new_weights_sum + initial_step_size)  # Số bước lặp hiện tại
        w = 3
        gamma = 0.75
        def update_gamma(current_gamma):
            """Nhánh TRUE: Cập nhật gamma."""
            return current_gamma * 0.997

        def keep_gamma(current_gamma):
            """Nhánh FALSE: Giữ nguyên gamma."""
            return current_gamma
        is_restart_step = (solver_state.num_iterations % 10000 == 0) & (solver_state.num_iterations > 50000)
        # Sử dụng jax.lax.cond để chọn giá trị gamma mới
        # cond(predicate, true_function, false_function, operand)
        gamma = jax.lax.cond(
            is_restart_step,
            update_gamma,
            keep_gamma,
            gamma  # `gamma` hiện tại được truyền vào làm toán hạng (operand)
        ) 
        weight = (solver_state.num_iterations + w + 1) / (solver_state.num_iterations + 2 * w + 2)
        next_primal_solution = (
            gamma * weight * (2*delta_primal)
            + weight * solver_state.current_primal_solution
            + (1 - weight) * solver_state.initial_primal_solution
        )
        next_primal_product = (
            gamma * weight * ( 2*delta_primal_product)
            + weight * solver_state.current_primal_product
            + (1 - weight) * solver_state.initial_primal_product
        )
        next_dual_solution = (
            gamma * weight * ( 2*delta_dual)
            + weight * solver_state.current_dual_solution
    ...
    def perform_restart(
        ...
        weight = (w+1)/(solver_state.num_iterations+w+1)
